    ...
    for (auto detectorType : detectorarray)
    {
        for (auto descriptorType : descriptorarray)
        {
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
            {
                ...
                detectObjects(dataRingBuffer.head()->cameraImg, dataRingBuffer.head()->boundingBoxes, confThreshold, nmsThreshold,
                ...
                dataRingBuffer.head()->lidarPoints = lidarPoints;
                ...
                clusterLidarWithROI(dataRingBuffer.head()->boundingBoxes, dataRingBuffer.head()->lidarPoints, shrinkFactor, P_rect_00, R_rect_00, RT);

                // Visualize 3D objects
                bVis = true;
                if (bVis)
                {
                    show3DObjects(dataRingBuffer.head()->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000), false);
                }
                bVis = false;
                ...
                cv::Mat imgGray;
                cv::cvtColor(dataRingBuffer.head()->cameraImg, imgGray, cv::COLOR_BGR2GRAY);
                ...
                if (dataRingBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    matchBoundingBoxes(matches, bbBestMatches, *dataRingBuffer.head(1), *dataRingBuffer.head()); // associate bounding boxes between current and previous frame using keypoint matches
                                                                                                                 //// EOF STUDENT ASSIGNMENT

                    // store matches in current data frame
                    dataRingBuffer.head()->bbMatches = bbBestMatches;
                    ...
                    for (auto it1 = dataRingBuffer.head()->bbMatches.begin(); it1 != dataRingBuffer.head()->bbMatches.end(); ++it1)
                    {
                        // find bounding boxes associates with current match
                        BoundingBox *prevBB, *currBB;
                        for (auto it2 = dataRingBuffer.head()->boundingBoxes.begin(); it2 != dataRingBuffer.head()->boundingBoxes.end(); ++it2)
                        {
                        ...
                        for (auto it2 = dataRingBuffer.head(1)->boundingBoxes.begin(); it2 != dataRingBuffer.head(1)->boundingBoxes.end(); ++it2)
                        {
                        ...
                        if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                        {
                            ...
                            double ttcCamera;
                            clusterKptMatchesWithROI(*currBB, dataRingBuffer.head(1)->keypoints, dataRingBuffer.head()->keypoints, dataRingBuffer.head()->kptMatches);
                            computeTTCCamera(dataRingBuffer.head(1)->keypoints, dataRingBuffer.head()->keypoints, currBB->kptMatches, sensorFrameRate, ttcCamera);
                            //// EOF STUDENT ASSIGNMENT
                            cout << imgIndex<< "-" <<ttcCamera << endl;
                            bVis = false;
                            if (bVis)
                            {
                                cv::Mat visImg = dataRingBuffer.head()->cameraImg.clone();
