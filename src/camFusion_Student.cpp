...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    // calculate Euclidean distances between keypoints from current and previous frame
                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
    ...
    // e.g. implement a custom bubble or quicksort algorithmus just for a percentage of the points.
    ...
    if (fabs(lidarPointsPrev[0].x - prevMeanX) > 0.15)
        cout << "OUTLIER DETECTED------------------- ";
    for (int i = 0; i < 10; i++)
    {
        //cout << "prev" << lidarPointsPrev[i].x << "current" << lidarPointsCurr[i].x << endl;
    }
    // compute TTC in accordance with the constant velocity motion model
    double dT = 1.0 / frameRate;
    //TTC = currMeanX * dT / (prevMeanX - currMeanX);
    //TTC = lidarPointsCurr.begin()->x * dT / (lidarPointsPrev.begin()->x - lidarPointsCurr.begin()->x);
    TTC = (lidarPointsCurr.begin()+3)->x * dT / ((lidarPointsPrev.begin()+3)->x - (lidarPointsCurr.begin()+3)->x);
    //cout << "prev" << (lidarPointsPrev.begin())->x << "current" << (lidarPointsCurr.begin())->x << endl;
    //cout << "Lidar TTC | " << TTC << endl;
}

static std::vector<int>
findBoundingBoxesWithKeypoint(const cv::KeyPoint &kpt, const std::vector<BoundingBox> &bounding_boxes)
{
    std::vector<int> filtered_bounding_boxes;
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    for (size_t i = 0; i < minBox; ++i)
    {
        ...
        for (size_t j = 0; j < maxBox; ++j)
        {
            ...
            if (minPrev)
            {
                //std::cout << "minprev " << count_table.at<int>(i, j) << "\n";
            ...
            {
                //std::cout << " maxprev" << count_table.at<int>(j, i) << "\n";
